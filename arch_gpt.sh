...
# Официально: конфиг — это файл /etc/snapper/configs/<name>; `create-config` обычно сам
# создаёт /.snapshots и прописывает SNAPPER_CONFIGS, но упадёт, если /.snapshots уже есть.
# Делаем «ручную» инициализацию, сохраняем минимализм и идемпотентность.
log "Configuring Snapper (idempotent; no baseline snapshots here)"

# 0) Подготовка каталога с конфигами
install -d /etc/snapper/configs

# helper: добавить имена конфигов в /etc/conf.d/snapper (SNAPPER_CONFIGS="root home"), идемпотентно
_ensure_sn_configs() {
  ...
  # текущее значение (может быть пустым)
  local cur
  cur="$(sed -n -E 's/^SNAPPER_CONFIGS="(.*)"/\1/p' "$f")"

  # комбинируем: текущее + новые, затем удаляем дубли
  local combined
  combined="${cur:+$cur }$*"
...
# helper: создать валидный конфиг-файл (с жёстной проверкой параметров под set -u)
_make_cfg() {
...
if btrfs subvolume show /.snapshots &>/dev/null; then
  # Отдельный сабвольюм @snapshots уже смонтирован в /.snapshots -> делаем конфиг вручную
  log "Snapper: /.snapshots exists -> writing /etc/snapper/configs/root manually"
  _make_cfg root /
else
  # Классический путь: позволим snapper'у создать /.snapshots и шаблон конфига
...
# 4) Включаем системные таймеры Snapper (timeline/cleanup) — без systemctl
...
# 5) Базовые снимки здесь НЕ создаём — ты сделаешь их вручную уже в установленной системе:
